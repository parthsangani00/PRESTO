...
class CIFAR10_ROTATE_OBJECTD(Dataset):
    def __init__(self, train):
        super().__init__()
        if train:
            seed_torch(0)
        ...
        if self.train:
            self.data_size = 25000
            # cifar = torch.load('../18_cifar/PyTorch_CIFAR10/cifar-rotate_without_pca_l4_half.pth')
            cifar = torch.load('./PyTorch_CIFAR10-ROTATE/cifar-rotate_objclassify_without_pca_l4_10C_half_2.pth')
            # self.data = torch.Tensor(cifar['final_train_features'])
            self.data = torch.Tensor(cifar['resnet18_train_features'])
            self.labels = torch.Tensor(cifar['train_labels'])
            self.labels = self.labels.type(torch.LongTensor)
...
class CIFAR10_ROTATE_THREEWAY(Dataset):
    def __init__(self, train):
        super().__init__()
        if train:
            seed_torch(0)
        ...
        self.num_classes = 3
        if self.train:
            self.data_size = 37500
            # cifar = torch.load('../18_cifar/PyTorch_CIFAR10/cifar-rotate_without_pca_l4_half.pth')
            cifar = torch.load('./PyTorch_CIFAR10-ROTATE/cifar-rotate_threeway_without_pca_l4_10C_half_2.pth')
            # self.data = torch.Tensor(cifar['final_train_features'])
            self.data = torch.Tensor(cifar['resnet18_train_features'])
            self.labels = torch.Tensor(cifar['train_labels'])
            self.labels = self.labels.type(torch.LongTensor)
...
class CIFAR100_ROTATE_OBJECTD(Dataset):
    def __init__(self, train):
        super().__init__()
        if train:
            seed_torch(0)
        ...
        if self.train:
            self.data_size = 25000
            # cifar = torch.load('../18_cifar/PyTorch_CIFAR10/cifar-rotate_without_pca_l4_half.pth')
            cifar = torch.load('./PyTorch_CIFAR100-ROTATE/cifar100-rotate_objclassify_without_pca_l4_10C_half_2.pth')
            # self.data = torch.Tensor(cifar['final_train_features'])
            self.data = torch.Tensor(cifar['resnet18_train_features'])
            self.labels = torch.Tensor(cifar['train_labels'])
            self.labels = self.labels.type(torch.LongTensor)
...
class CIFAR100_ROTATE_THREEWAY(Dataset):
    def __init__(self, train):
        super().__init__()
        if train:
            seed_torch(0)
        ...
        if self.train:
            self.data_size = 37500
            # cifar = torch.load('../18_cifar/PyTorch_CIFAR10/cifar-rotate_without_pca_l4_half.pth')
            cifar = torch.load('./PyTorch_CIFAR100-ROTATE/cifar-rotate_threeway_without_pca_l4_10C_half_2.pth')
            # self.data = torch.Tensor(cifar['final_train_features'])
            self.data = torch.Tensor(cifar['resnet18_train_features'])
            self.labels = torch.Tensor(cifar['train_labels'])
            self.labels = self.labels.type(torch.LongTensor)
...
class NVD_ROTATE(Dataset):
    def __init__(self, train):
        ...
        self.num_classes = 2
        if self.train:
            self.data_size = 16000
            nvd = torch.load('./NVD/nvd-rotate_without_pca_l4_10K.pth')
            self.data = torch.Tensor(nvd['resnet18_train_features'])
            self.labels = torch.Tensor(nvd['train_labels'])
            self.labels = self.labels.type(torch.LongTensor)
        else:
            self.data_size = 4800
            nvd = torch.load('./NVD/nvd-rotate_without_pca_l4_10K.pth')
            # self.data = torch.Tensor(cifar['final_test_features'])
            self.data = torch.Tensor(nvd['resnet18_test_features'])
            self.labels = torch.Tensor(nvd['test_labels'])
            self.labels = self.labels.type(torch.LongTensor)
...
class VisualGenome_ROTATE(Dataset):
    def __init__(self, train):
        super().__init__()
        if train:
            seed_torch(0)
        ...
        if self.train:
            self.data_size = 49900
            vg = torch.load('./PyTorch_VisualGenome-ROTATE/visualgenome-rotate_without_pca_l4_50K.pth')
            self.data = torch.Tensor(vg['resnet18_train_features'])
            self.labels = torch.Tensor(vg['train_labels'])
            self.labels = self.labels.type(torch.LongTensor)
        else:
            self.data_size = 13900
            vg = torch.load('./PyTorch_VisualGenome-ROTATE/visualgenome-rotate_without_pca_l4_50K.pth')
            # self.data = torch.Tensor(cifar['final_test_features'])
            self.data = torch.Tensor(vg['resnet18_test_features'])
            self.labels = torch.Tensor(vg['test_labels'])
            self.labels = self.labels.type(torch.LongTensor)
...
class STL10_ROTATE_OBJECTD(Dataset):
    def __init__(self, train):
        super().__init__()
        if train:
            seed_torch(0)
        ...
        if self.train:
            self.data_size = 16000*2
            cifar = torch.load('./PyTorch_STL10-ROTATE/stl_objclassify_l4_32_10K_half.pth')
            self.data = torch.Tensor(cifar['resnet18_train_features'])
            self.labels = torch.Tensor(cifar['train_labels'])
            self.labels = self.labels.type(torch.LongTensor)
        else:
...
class StanfordCars_ROTATE(Dataset):
    def __init__(self, train):
        super().__init__()
        if train:
            seed_torch(0)
        ...
        if self.train:
            self.data_size = 8144
            # cifar = torch.load('../18_cifar/PyTorch_CIFAR10/cifar-rotate_without_pca_l4_half.pth')
            cars = torch.load('./PyTorch_StanfordCARS/stanfordcars-rotate_without_pca_l4_196C.pth')
            # self.data = torch.Tensor(cifar['final_train_features'])
            self.data = torch.Tensor(cars['resnet18_train_features'])
            self.labels = torch.Tensor(cars['train_labels'])
            self.labels = self.labels.type(torch.LongTensor)
        else:
            self.data_size = 8041
            # cifar = torch.load('../18_cifar/PyTorch_CIFAR10/cifar-rotate_without_pca_l4_half.pth')
            cars = torch.load('./PyTorch_StanfordCARS/stanfordcars-rotate_without_pca_l4_196C.pth')
            # self.data = torch.Tensor(cifar['final_test_features'])
            self.data = torch.Tensor(cars['resnet18_test_features'])
            self.labels = torch.Tensor(cars['test_labels'])
            self.labels = self.labels.type(torch.LongTensor)
