...
if __name__ == "__main__":
    ...
    def train_skyline(trainset, trainloader, testset, testloader, max_gpu_usage, is_baseline, hidden_dim1, hidden_dim2, hidden_dim3):
        ...
        torch.save(dic, checkpoint_path + 'skyline.pth')
    ...
    def train_baseline(skyline_classifier, trainset, trainloader, testset, testloader, max_gpu_usage, is_baseline, hidden_dim1, hidden_dim2, hidden_dim3, get_param_count=False):
        ...
        for epoch in range(num_epochs):
            ...
            for _,(index,data,labels) in enumerate(trainloader):
                ...
                with torch.no_grad():
                    skyline_outputs = skyline_classifier(data)

                loss_KL = nn.KLDivLoss()(F.log_softmax(outputs / T, dim=1), F.softmax(skyline_outputs / T, dim=1))
        ...
        dic={}
        dic['student'] = classifier.state_dict()
        torch.save(dic, checkpoint_path + 'student.pth')
    ...
    if 'rotate' in args.dataset or args.dataset == 'cifar10':
        if args.get_param_count:
            baseline_classifier = train_baseline(None, trainset, trainloader, testset, testloader, max_gpu_usage, is_baseline=True, hidden_dim1=args.hidden_dim_1, hidden_dim2=args.hidden_dim_2, hidden_dim3=args.hidden_dim_3,
                                                 get_param_count=True)
            params = sum(p.numel() for p in baseline_classifier.parameters() if p.requires_grad)
            print("The number of parameters is -: ", params)
            sys.exit(0)

        skyline_classifier, max_gpu_usage = train_skyline(trainset, trainloader, testset, testloader, max_gpu_usage, is_baseline=False, hidden_dim1=256, hidden_dim2=512, hidden_dim3=0) #skyline
        max_gpu_usage = train_baseline(skyline_classifier, trainset, trainloader, testset, testloader, max_gpu_usage, is_baseline=True, hidden_dim1=args.hidden_dim_1, hidden_dim2=args.hidden_dim_2, hidden_dim3=args.hidden_dim_3)
    else:
        skyline_classifier, max_gpu_usage = train_skyline(trainset, trainloader, testset, testloader, max_gpu_usage, is_baseline=False, hidden_dim1=128, hidden_dim2=256, hidden_dim3=512) #skyline
        max_gpu_usage = train_baseline(skyline_classifier, trainset, trainloader, testset, testloader, max_gpu_usage, is_baseline=True, hidden_dim1=12, hidden_dim2=16, hidden_dim3=32)

    print("Max GPU Usage : ", max_gpu_usage)

    # if not args.clustering_baseline:
    #     skyline_classifier, max_gpu_usage = train_skyline(trainset, trainloader, testset, testloader, max_gpu_usage, is_baseline=False, hidden_dim1=128, hidden_dim2=256, hidden_dim3=512) #skyline
    #     max_gpu_usage = train_baseline(skyline_classifier, trainset, trainloader, testset, testloader, max_gpu_usage, is_baseline=True, hidden_dim1=16, hidden_dim2=24, hidden_dim3=32)

    #     print("Max GPU Usage : ", max_gpu_usage)

    #     # teacher -> 128,256,512 (always)
    #     # student -> 4,8,16 (in paper table)
    #     # student -> 12,16,24 (matches the PRESTO params for k = 3)
    #     # student -> 12,16,32 (matches the PRESTO params for k = 4)
    #     # student -> 16,24,32 (matches the PRESTO params for k = 5)
    #     # student -> 20,26,32 (matches the PRESTO params for k = 6)
